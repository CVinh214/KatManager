...
- `SESSION_SECRET` — khóa ký cookie phiên đăng nhập (bắt buộc trên production)

API Security

- API được bảo vệ theo 2 lớp:
  - Middleware tập trung tại `src/middleware.ts` chặn request không có session hợp lệ và kiểm tra quyền manager cho endpoint quản trị.
  - Guard trong từng route kiểm tra chi tiết hơn (ví dụ staff chỉ thao tác dữ liệu của chính mình).
- Endpoint công khai không yêu cầu đăng nhập: `/api/auth/login`, `/api/auth/logout`, `/api/holidays`.
...
_Ghi chú:_ README này là bản tóm tắt hiện trạng;
