...
export async function GET(request: NextRequest) {
  try {
    ...
    if (employeeId) {
      ...
      if (auth.user.role === "staff" && auth.user.employeeId !== employeeId) {
        return NextResponse.json({ error: "Forbidden" }, { status: 403 });
      }
    } else if (auth.user.role === "staff") {
      if (!auth.user.employeeId) {
        return NextResponse.json(
          { error: "Staff account is not linked to employee" },
          { status: 403 },
        );
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!acquireLock(lockKey)) {
      console.warn(
        `[Duplicate Prevention] Blocked duplicate shift preference submission for ${lockKey}`,
      );
      return NextResponse.json(
    ...
    try {
      ...
      if (!isOff && (!startTime || !endTime)) {
        return NextResponse.json(
          {
            error:
              "Missing required fields: startTime and endTime are required for work shifts",
          },
          { status: 400 },
        );
      }
      ...
      if (employeeId.startsWith("emp-")) {
        console.warn(
          "Legacy employeeId detected, attempting to find employee by other means",
        );
      ...
      const out = {
        ...preference,
        date: formatDateISO(preference.date as Date),
      };
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (auth.user.role === "staff") {
      ...
      if (!existing || existing.employeeId !== auth.user.employeeId) {
        return NextResponse.json({ error: "Forbidden" }, { status: 403 });
      }
      if (status !== undefined) {
        return NextResponse.json(
          { error: "Only manager can change approval status" },
          { status: 403 },
        );
      }
    }
